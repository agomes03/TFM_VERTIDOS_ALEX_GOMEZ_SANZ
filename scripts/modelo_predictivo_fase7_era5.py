...
pr_curve.to_csv(OUT_DIR / "fase7_curva_precision_recall_era5.csv", index=False)

importances.to_csv(OUT_DIR / "fase7_importancia_variables_era5.csv", index=False)
...
test_out.to_csv(OUT_DIR / "fase7_predicciones_2026_era5.csv", index=False)

with open(OUT_DIR / "fase7_resumen_era5.txt", "w", encoding="utf-8") as f:
...
print(f"\nGuardado en {OUT_DIR}: fase7_predicciones_2026_era5.csv, fase7_importancia_variables_era5.csv, "
      f"fase7_curva_precision_recall_era5.csv, fase7_resumen_era5.txt")
